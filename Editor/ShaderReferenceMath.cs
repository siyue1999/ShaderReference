        ...
        public void DrawMainGUI()
        {
            ...
            switch (ShaderReferenceEditorWindow.mPipeline)
            {
                ...
                case ShaderReferenceEditorWindow.Pipline.URP:
                    ...
                    ShaderReferenceUtil.DrawOneContent("SAMPLE_TEXTURE2D_ARRAY_LOD(textureName, samplerName, coord2, index, lod);", "纹理数组采样.\n" + "textureName:Properties中声明的2D纹理名称\n" + "samplerName:此纹理所使用的采样器设置\n" + "coord2:采样用的UV\n" + "index:数组索引,必须用int,否则部分机型上会变粉\n"+"lod:指定Mipmap级别");
                    ShaderReferenceUtil.DrawOneContent("SAMPLE_TEXTURE2D_ARRAY_BIAS(textureName, samplerName, coord2, index, bias);", "纹理数组采样.\n" + "textureName:Properties中声明的2D纹理名称\n" + "samplerName:此纹理所使用的采样器设置\n" + "coord2:采样用的UV\n" + "index:数组索引,必须用int,否则部分机型上会变粉\n"+"bias:指定Mipmap偏移级别,负值更清晰");
                    break;
            }
